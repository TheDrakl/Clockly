...
function UserBook() {
    ...
    useEffect(() => {
        const getServices = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:8000/api/bookings/services/${username}/`, {
                    method: "GET",
    ...
    const handleContinue = async () => {
        if (selectedDate) {
            ...
            try {
                ...
                const response = await fetch(`http://127.0.0.1:8000/api/bookings/${username}/${activeService}/${formattedDate}/`, {
                    method: "GET",
                    headers: { 'Content-Type': 'application/json' }
                })
    ...
    const handleBookingSubmit = async (e) => {
        ...
        try {
            const formattedDate = selectedDate.toISOString().split('T')[0]
            const response = await fetch(`http://127.0.0.1:8000/api/bookings/book/${username}/${activeService}/${formattedDate}/`, {
                method: 'POST',
                headers: {
